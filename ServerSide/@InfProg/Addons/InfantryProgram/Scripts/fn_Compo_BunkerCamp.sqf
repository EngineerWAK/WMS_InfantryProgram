...
]; //this could be changed by a server variable like 'private _objects = WMS_object_compo_bunkerCamp', same for the units (MG) datas.
...
uisleep 0.5; //delay between every objects creation
} forEach _objects; 
WMS_Compo_BunkerCamp_LastTime = time;
publicVariable "WMS_Compo_BunkerCamp_LastTime";
 
_campGrp1 = createGroup [BLUFOR, false]; //this one outside to help with the MultigroupDelete
if (_armed == 1) then {
