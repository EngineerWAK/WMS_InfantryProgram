...
[
	...
	] call WMS_fnc_DynAI_SetUnitOPF;

*/
if (WMS_DynAI_LOGs) then {diag_log format ["[OPFOR AI SETUP]|WAK|TNA|WMS| _this = %1", _this]};
...
params[
	...
	["_info", 'nothingYet'], //"BaseATK" for EH ok kill reinforcement //nope
	["_difficulty", "difficult"]
];
if (WMS_DynAI_LOGs) then {diag_log format ["[OPFOR AI SETUP]|WAK|TNA|WMS| _units = %1", _units]};
...
_sniper = [1,0.95,0.95,0.95];
{
	...
	_poptabs = (WMS_DynAI_poptabsINF select 0) + round (random(WMS_DynAI_poptabsINF select 1)); //for Exile Mod
	...
	if((random 100) <= _launcherChance) then { _launcher = [_unit, selectrandom (WMS_AI_LaunchersOPF select 0), 1] call BIS_fnc_addWeapon;};
