...
if (WMS_InfantryProgram_LOGs) then {diag_log format ["[PARADROP GROUP]|TNA|TNA|TNA|TNA|TNA| _this = %1", _this]};
...
if (_grpSide == OPFOR ) then {
	if (WMS_InfantryProgram_LOGs) then {diag_log format ["[PARADROP GROUP]|TNA|TNA|TNA|TNA|TNA| Side = %1", _grpSide]};
	_smokeGrenade = "SmokeShellRed";
	[(units _paraGrp),'Para',_launcherChance,_skill,_loadout,nil,nil,_difficulty] call WMS_fnc_DynAI_SetUnitOPF;
...
if (_grpSide == BLUFOR ) then {
	...
	//[(units _paraGrp),'SMG',_launcherChance,_skill,_loadout] call WMS_fnc_DynAI_SetUnitBLU;
	{[_x,'Para',_launcherChance,_skill,_loadout] call WMS_fnc_DynAI_SetUnitBLU}forEach (units _paraGrp);
	{
...
//[_paraGrp, _pos, _WPDist, 5, _WPType, _WPBeha, _WPComb, WPS, "STAG COLUMN", "", [1,2,3]] call CBA_fnc_taskPatrol; //OLD
if (WMS_DynAI_Steal) then {
	...
	[_paraGrp, _pos, _WPDist, 5, _WPType, _WPBeha, _WPComb, _WPSpee, "COLUMN", "", [1,2,3]] call CBA_fnc_taskPatrol; //NEW //if (WMS_DynAI_Steal) then {[_grp] call WMS_DynAI_steal};
};
