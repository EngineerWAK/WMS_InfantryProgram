...
if (WMS_Events_LOGs) then {diag_log format ["[EVENTS WATCH]|WAK|TNA|WMS| Next Event: %1 @%2",_type,_timeToWait];};
if (time > _timeToWait) then {
	if (_type == "RepairCamp") then {//Repair Camp has nothing to watch, no WMS_Events_Running pushback
		...
		if (WMS_Events_LOGs) then {diag_log format ["[EVENTS WATCH]|WAK|TNA|WMS| Processing Event: %1 @%2", _type, _pos];};
...
if (count WMS_Events_Running != 0) then { 
	{ 
		if (WMS_Events_LOGs) then {diag_log format ["[ENEMY SUPPLYDROP Watch]|WAK|TNA|WMS| %1 Events Monitored", count WMS_Events_Running]};
		...
		if (((_x select 7) == "supplydrop") && {(count ((position (leader (_x select 2 select 0))) nearEntities [WMS_PlayerEntity, WMS_AI_PlayerDistToDespawnINF]) == 0)}) then {
			...
			if (_cnt == 0 && {(_opened)} && {(_openedTime > time+300)}) then {
				if (WMS_Events_LOGs) then {diag_log format ["[ENEMY SUPPLYDROP Watch]|WAK|TNA|WMS| %1 to be deleted", _x]};
