...
/*for "_i" from 1 to WMS_AI_RoamingVHLcount do {
	...
	[_pos, "", WMS_AI_PatrolTimer_VHL+(random 300),_skill,OPFOR,_loadout,_vhlFull,false,false,0,150,WMS_AI_VHLwptDist,"MOVE","SAFE","YELLOW","LIMITED",_infoType] spawn WMS_fnc_infantryProgram_VHLpatrol;

	if (WMS_IP_LOGs) then { diag_log format ["[240 sec AI VHL SPAWN]|WAK|TNA|WMS| spawning %1 position %2", _vhl,_pos]};
	uisleep 30;
};*/
