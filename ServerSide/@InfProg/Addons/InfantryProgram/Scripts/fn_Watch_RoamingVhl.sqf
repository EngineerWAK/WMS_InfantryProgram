...
if !(_RoamingAIvhl == 0) then { 
	if (WMS_AI_RoamingVhl_Logs) then {diag_log format ["[Roaming VHL Watch]|WAK|TNA|WMS| %1 Vehicles Patroling", _RoamingAIvhl]};
	{ 
			...
			if ((time > (_timeAdded+60)) && {alive (_x select 0)} && {side(group(_x select 0)) == EAST} && {alive (driver (_x select 0))} && {count ((position (_x select 0)) nearEntities [WMS_PlayerEntity, 300]) == 0}) then { //side EAST means there is still AI inside otherwise side UNKNOW
				...
				if ((_newPos distance2d _lastPos) < 3) then {
					...
					if (_assKicked < WMS_AI_RoamingVHL_MaxKick) then {
						...
						if (WMS_AI_RoamingVhl_Logs) then {diag_log format ["[Roaming VHL Watch]|WAK|TNA|WMS| moving %1 from %2 to %3, assKicked %4 times", _vhl, _newPos, _closeSafePos,_assKicked]};
					} else {
						_vhl setDamage 1;
						if (WMS_AI_RoamingVhl_Logs) then {diag_log format ["[Roaming VHL Watch]|WAK|TNA|WMS| %1 is a moron and is now destroyed", _vhl]};
					};
				};
			_x set [1, _newPos];
			} else {
				if (isplayer(driver(_x select 0)) || (owner (_x select 0) != 2)) then { //if player is driving, remove the vehicle from the cleanup list
					if (WMS_AI_RoamingVhl_Logs) then {diag_log format ["[Roaming VHL OWNERS]|WAK|TNA|WMS| vehicle: %1 owned by a player and removing it from the list",(_x select 0)]};
		...
		if ((count units ((_x select 2) select 0)) < 1) then {
			if (WMS_AI_RoamingVhl_Logs) then {diag_log format ["[Roaming VHL Watch]|WAK|TNA|WMS| %1 is no more, rest in peace and get the fuck out of the Array", _x]};
			_x set [1,0];
		};
		/////Unstuck vehicle
		...
		if (time > (_x select 1) && {count ((position (leader (_x select 2 select 0))) nearEntities [WMS_PlayerEntity, WMS_AI_PlayerDistToDespawnVHL]) == 0}) then {//totaly not sure this filter is doing the job
			if (WMS_AI_RoamingVhl_Logs) then {diag_log format ["[Roaming VHL Watch]|WAK|TNA|WMS| %1 to be deleted", _x]};
			...
			} forEach _grpArray; 
			{
				...
				if (WMS_AI_RoamingVhl_Logs) then {diag_log format ["[Roaming VHL OWNERS]|WAK|TNA|WMS| vehicle: %1 owned by: %2",(_x select 0), _ownedBy]};
				if (isplayer(driver(_x select 0)) || (_ownedBy != 2)) then {
					if (WMS_AI_RoamingVhl_Logs) then {diag_log format ["[Roaming VHL OWNERS]|WAK|TNA|WMS| vehicle: %1 owned by a player",(_x select 0)]};
				} else {
...
if !(_RoamingAIair == 0) then { 
	{ 
		...
		if (time > (_x select 1) && {count ((position (leader (_x select 2 select 0))) nearEntities [WMS_PlayerEntity, WMS_AI_PlayerDistToDespawnAIR]) == 0}) then {
			if (WMS_AI_RoamingVhl_Logs) then {diag_log format ["[Roaming AIR Watch]|WAK|TNA|WMS| %1 to be deleted", _x]};
			_grpArray = (_x select 2);
			_obj = (_x select 4);
			_mkr = (_x select 5);
			_wps = (_x select 6);
			...
			{
				_ownedBy = (owner (_x select 0));
				if (WMS_AI_RoamingVhl_Logs) then {diag_log format ["[Roaming AIR OWNERS]|WAK|TNA|WMS| vehicle: %1 owned by: %2",(_x select 0), _ownedBy]};
				if (isplayer(driver(_x select 0)) || (_ownedBy != 2)) then {
					if (WMS_AI_RoamingVhl_Logs) then {diag_log format ["[Roaming AIR OWNERS]|WAK|TNA|WMS| vehicle: %1 owned by a player",(_x select 0)]};
					PlaySound3D ["A3\Sounds_F\sfx\hint-4.wss", (_x select 0), false, position (_x select 0), 2, 1, 0];
