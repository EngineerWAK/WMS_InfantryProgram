...
if (WMS_AMS_LOGs) then {diag_log format ["[Advanced Mission System STARTING]|WAK|TNA|WMS| WMS_AMS_MissionList: %1", WMS_AMS_MissionList]};
...
//Init the missions
{
	...
	if (WMS_AMS_LOGs) then {diag_log format ["[Advanced Mission System PROCESSING]|WAK|TNA|WMS| _missionName: %1 || _missionWeight %2 || _missionVarName deactivated", _missionName,_missionWeight]};//,(missionNamespace getVariable (format ["AMS_Mission_%1",_missionName]))
} forEach WMS_AMS_MissionList;

if (WMS_AMS_LOGs) then {diag_log format ["[Advanced Mission System PROCESSING]|WAK|TNA|WMS| WMS_AMS_MissionList: %1", WMS_AMS_Missions]};