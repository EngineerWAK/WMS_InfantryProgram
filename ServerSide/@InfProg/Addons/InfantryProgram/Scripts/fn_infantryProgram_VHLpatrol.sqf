...
//////////////////////////////////////////////////////////////////
if (WMS_InfantryProgram_LOGs) then {diag_log format ["[VHL PATROL]|WAK|TNA|WMS| _this = %1", _this]};
private ["_cargoCount","_VHLcount","_VHLcount0","_VHLcount1","_VHLselected","_markerName","_markerType","_randomPosTarget","_vhl","_VHLgrp","_vehic","_markerPos","_VHLMarker1","_WPT_1","_drvSits","_comSits","_gunSits"];
params[  
 "_pos",
 "_target",
 ["_timer", 600, [0]],
 ...
 ["_WPComb", "YELLOW", [""]],  
 ["_WPSpee", "NORMAL", [""]],  
 ["_infoType", "Patrol", [""]],  //not used yet, can use this to modify management _target = "Occupation";
 ["_difficulty", (selectRandom ["easy","moderate","difficult"])]

];
private _playerMoney = 0;
private _playerScore = 0;
//private _VHLfull = selectRandom _VHLfull;
...
if (_drvSits != 0) then {
	WMS_AI_Units_Class createUnit [_randomPosTarget, _VHLgrp, "this moveinDriver _vehic"];
};
/////
_comSits = _vehic emptyPositions "Commander";
_gunSits = _vehic emptyPositions "Gunner";
if (_comSits != 0) then {
	WMS_AI_Units_Class createUnit [_randomPosTarget, _VHLgrp, "this moveinCommander _vehic"];
};
if (_gunSits != 0) then {
	WMS_AI_Units_Class createUnit [_randomPosTarget, _VHLgrp, "this moveinGunner _vehic"];
};
if (WMS_InfantryProgram_LOGs) then {diag_log format ["[VHL PATROL]|WAK|TNA|WMS| Vehicle: %1, Cargo: %2, Driver: %3, Commander: %4, _Gunner: %5, AllTurret: %6", _VHLselected, _cargoSits,_drvSits,_comSits,_gunSits, (count allTurrets _vehic)]};
...
if (_grpSide == OPFOR ) then {
	if ((_cargoSits > 0) && WMS_AI_AddCargoUnits && {(OPFOR countSide allUnits) < WMS_AI_MaxUnits_A} && {!(_VHLselected iskindof "helicopter")} && {!(_VHLselected iskindof "plane")}) then {
		if (_cargoSits >= WMS_AI_CargoUnits) then {
			for "_i" from 1 to WMS_AI_CargoUnits do {
				WMS_AI_Units_Class createUnit [_randomPosTarget, _VHLgrp, "this moveincargo _vehic"];
			};
		} else {
	...
	[(units _VHLgrp),'SMG',WMS_DynAI_LauncherChance,_skill,_loadout,nil,nil,_difficulty] call WMS_fnc_DynAI_SetUnitOPF;
	...
	if (WMS_exileFireAndForget) then {
		...
		_vehic addMPEventHandler ["mpkilled", {
				...
				if (isPlayer (_this select 1)) then {
					//[(_this select 1), "ROAMING"] remoteExec ['WMS_fnc_AI_rewardOnVHLdestroy', 2];
...
if (_grpSide == BLUFOR ) then {
	{[_x,'SMG',10,_skill,_loadout] call WMS_fnc_DynAI_SetUnitBLU}forEach (units _VHLgrp);
	if (_useMarker) then {
...
_WPT_1 = _VHLgrp addWaypoint [_pos, 100];
...
if (_VHLselected iskindof "helicopter" || _VHLselected iskindof "plane") then {
	WMS_AI_RoamingAIR_Running pushback [time,(time+(_timer+(random _timer))),[_VHLgrp],[[_vehic,(position _vehic)]],[],[_markerName],[_WPT_1],""];
	} else {
	WMS_AI_RoamingVHL_Running pushback [time,(time+(_timer+(random _timer))),[_VHLgrp],[[_vehic,(position _vehic)]],[],[_markerName],[_WPT_1],""];
};
switch (_infoType) do {
	case "Occupation" : {
...
waituntil {(_vehic distance2D _pos) <= 300;};
uisleep 15;
//[_VHLgrp, _pos, _WPDist, 5, _WPType, _WPBeha, _WPComb, _WPSpee, "COLUMN", "", [0,0,0]] call CBA_fnc_taskPatrol;
