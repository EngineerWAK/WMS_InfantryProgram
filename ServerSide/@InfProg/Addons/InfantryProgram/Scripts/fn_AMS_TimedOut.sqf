...
private _pos = GetPosATL (objectFromNetID (_objs select 0));
...
playSound3D [getMissionPath	'Custom\Ogg\germanwin.ogg', objectFromNetID(_objs select 0), false, _pos, 2, 1, 0];
{{deleteVehicle _x} foreach units _x} forEach _grps; 
{deleteVehicle _x} forEach _vhls;  
{deleteVehicle (objectFromNetID _x)} forEach _objs;  
