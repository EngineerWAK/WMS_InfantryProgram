...
if (WMS_Pos_AutoScan) then {
	...
	{WMS_Pos_Hills pushback getPos _x}forEach (nearestLocations [_worldCenter, ["hill"],_worldDiameter]);
	if (WMS_CustomTrig_activate) then {
		Diag_log '|WAK|TNA|WMS| collecting CUSTOM positions';
		{WMS_Pos_CustomTrig pushback getPos _x}forEach (nearestObjects [_worldCenter, [WMS_CustomTrig_ClassName],_worldDiameter]);
	};
