/**
* InfantryProgram
*
* TNA-Community
* https://discord.gg/Zs23URtjwF
* © 2021 {|||TNA|||}WAKeupneo
*
* This work is licensed under the Creative Commons Attribution-NonCommercial-NoDerivatives 4.0 International License. 
* To view a copy of this license, visit http://creativecommons.org/licenses/by-nc-nd/4.0/.
* Do Not Re-Upload
*/
/////////////////////////////////
//			CRATES ITEMS
/////////////////////////////////
WMS_SupplyList = [
	"Exile_Item_WoodPlank",
	"Exile_Item_Cement",
	"Exile_Item_Sand",
	"Exile_Item_JunkMetal",
	"Exile_Item_WaterCanisterDirtyWater",
	"Exile_Item_FuelCanisterFull"
];
WMS_ToolList = [
	//"Exile_Item_Foolbox",
	"Exile_Item_DuctTape",
	"Exile_Item_ExtensionCord",
	"Exile_Item_Grinder",
	"Exile_Item_Handsaw",
	"Exile_Item_MetalScrews",
	"Exile_Item_MetalWire",
	"Exile_Item_Pliers",
	"Exile_Item_Rope",
	"Exile_Item_Screwdriver",
	"Exile_Item_Shovel",
	"Exile_Item_CookingPot",
	"Exile_Item_LightBulb",
	"Exile_Item_Matches",
	"Exile_Item_CanOpener"
];
WMS_MeatList = [
	"Exile_Item_AlsatianSteak_Raw",
	"Exile_Item_CatSharkFilet_Raw",
	"Exile_Item_ChickenFilet_Raw",
	"Exile_Item_FinSteak_Raw",
	"Exile_Item_GoatSteak_Raw",
	"Exile_Item_MackerelFilet_Raw",
	"Exile_Item_MulletFilet_Raw",
	"Exile_Item_OrnateFilet_Raw",
	"Exile_Item_RabbitSteak_Raw",
	"Exile_Item_RoosterFilet_Raw",
	"Exile_Item_SalemaFilet_Raw",
	"Exile_Item_SheepSteak_Raw",
	"Exile_Item_SnakeFilet_Raw",
	"Exile_Item_TunaFilet_Raw",
	"Exile_Item_TurtleFilet_Raw"
];
WMS_medsList = [
	"Exile_Item_Bandage",
	"Exile_Item_Bandage",
	"Exile_Item_Bandage",
	"Exile_Item_Bandage",
	"Exile_Item_Bandage",
	"Exile_Item_Vishpirin",
	"Exile_Item_Vishpirin",
	"Exile_Item_Vishpirin",
	"Exile_Item_InstaDoc"
];
/////
WMS_IP_ToolKit = [
	["Exile_Item_OldChestKit",1,0],
	["Land_TentDome_F_Kit",1,0],
	["Exile_Item_CampFireKit",1,0],
	["Exile_Item_Matches",1,0],
	["Exile_Item_CookingPot",1,0],
	["Exile_Item_SafeSmallKit",1,0],
	["Exile_Item_Knife",1,0],
	["Exile_Item_EMRE",1,1],   
	["Exile_Item_PlasticBottleCoffee",1,1],   
	["Exile_Item_Bandage",1,3]
];
WMS_medicList = [
	["Exile_Item_Bandage",5,5],
	["Exile_Item_Vishpirin",1,3],
	["Exile_Item_InstaDoc",1,2]
];
WMS_foodList = [
	["Exile_Item_PlasticBottleFreshWater",4,4],
	["Exile_Item_InstantCoffee",2,2],
	["Exile_Item_BeefParts",2,2],
	["Exile_Item_EMRE",1,1]
];
WMS_miscList = [
	["Laserbatteries",2,3],
	["Exile_Item_CanOpener",1,1],
	["Exile_Item_CookingPot",0,3],
	["Exile_Item_DuctTape",1,2],
	["Exile_Item_Knife",0,2],
	["Exile_Item_MobilePhone",1,0],
	["Exile_Item_RubberDuck",1,5],
	["Exile_Item_Shovel",0,1],
	["Exile_Item_ToiletPaper",5,5],
	["Exile_Item_FuelCanisterFull",0,2],
	["SmokeShellGreen",1,5],
	["Exile_Item_FireExtinguisher",0,1]
];
WMS_weapsList = [
	["hgun_P07_F",1,0],
	["hgun_Pistol_Signal_F",0,1],
	["arifle_MXC_Black_F",0,1]
];
WMS_humaniDropList = [
	["EBM_Item_Bucket_Of_Mortar",25,10],
	["EBM_Item_Bricks",30,15],
	["Exile_Item_Sand",20,10],
	["Exile_Item_Cement",20,10],
	//["Exile_Item_CanOpener",5,5],
	//["Exile_Item_CookingPot",3,5],
	["Exile_Item_EMRE",5,10],
	["Exile_Item_BeefParts",5,5],
	["Exile_Item_PlasticBottleFreshWater",20,15],
	["Exile_Item_Bandage",10,20],
	["Exile_Item_ToiletPaper",10,10],
	["Exile_Item_FuelCanisterFull",10,5],
	["Exile_Item_PortableGeneratorKit",3,5],
	["Exile_Item_WaterCanisterEmpty",5,3]
];
